...
if ( ! empty( $atts['popup_pos'] ) ) {
	...
	$search_layout = ! empty( $atts['search_layout'] ) ? $atts['search_layout'] : $porto_settings['search-layout'];
	if ( 'simple' == $search_layout || 'large' == $search_layout ) {
		echo '#header .search-popup .searchform {';
		echo porto_filter_output( $popup_style );
		echo '}';
	} elseif ( 'advanced' == $search_layout ) {
