		...
		if (have_posts()):
			...
			<div class="<?php echo esc_attr($wrap_cls); ?>" <?php echo porto_filter_output($wrap_attrs); ?>>
				...
				<?php if (!empty($porto_settings['member-archive-ajax'])): ?>
					<div id="memberAjaxBox" class="ajax-box">
						...
						<?php if (function_exists('porto_title_archive_name') && porto_title_archive_name('member')): ?>
							...
							<div class="hide ajax-content-append">
								<h4 class="m-t-sm m-b-lg">
									<?php printf(esc_html__('More %s:', 'porto'), porto_title_archive_name('member')); ?></h4>
							</div>
						<?php endif; ?>
					</div>
				<?php endif; ?>
					...
					if ('sidebar' === $porto_settings['member-cat-sort-pos'] && !('widewidth' == $porto_layout || 'fullwidth' == $porto_layout)) {
						...
						if (is_array($member_taxs) && !empty($member_taxs)):
							...
							if ('widewidth' === $porto_layout):
								...
								<div class="container"><?php endif; ?>
								<ul
									class="member-filter nav sort-source <?php echo isset($porto_settings['member-cat-sort-style']) && $porto_settings['member-cat-sort-style'] ? 'sort-source-' . esc_attr($porto_settings['member-cat-sort-style']) : 'nav-pills', empty($porto_settings['member-cat-ft']) || empty($member_infinite) ? '' : ' porto-ajax-filter'; ?>">
									...
									<?php foreach ($member_taxs as $member_tax_slug => $member_tax_name): ?>
										<li data-filter="<?php echo esc_attr($member_tax_slug); ?>"><a
												href="<?php echo esc_url(get_term_link($member_tax_slug, 'member_cat')); ?>"><?php echo esc_html($member_tax_name); ?></a>
										</li>
									<?php endforeach; ?>
								</ul>
								<hr>
				...
				<?php else: ?>
					...
					<div
						class="member-row members-container row ccols-wrap <?php echo porto_generate_column_classes($member_columns); ?>">
						<?php
						while (have_posts()) {
							the_post();
