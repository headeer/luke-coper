	...
	?>">
		...
		<?php if (!is_search() && isset($porto_settings['portfolio-cat-sort-pos']) && 'content' == $porto_settings['portfolio-cat-sort-pos'] && !empty($porto_settings['portfolio-title'])): ?>
			...
			if ('widewidth' === $porto_layout):
				...
				<h2 class="portfolio-archive-title"><?php echo porto_strip_script_tags($porto_settings['portfolio-title']); ?>
				</h2>
				<?php
				if ('widewidth' === $porto_layout):
					?>
		...
		if (have_posts()):
			...
			if (!empty($porto_settings['portfolio-archive-link-zoom'])):
				?>
				<div
					class="portfolios-lightbox<?php echo empty($porto_settings['portfolio-archive-img-lightbox-thumb']) ? '' : ' with-thumbs'; ?>">
				<?php endif; ?>
						...
						if ('sidebar' === $porto_settings['portfolio-cat-sort-pos'] && !('widewidth' == $porto_layout || 'fullwidth' == $porto_layout)) {
							...
							if (is_array($portfolio_taxs) && !empty($portfolio_taxs)):
								...
								if ('widewidth' === $porto_layout):
									...
									<div class="container"><?php endif; ?>
									<ul
										class="portfolio-filter nav sort-source <?php echo isset($porto_settings['portfolio-cat-sort-style']) && $porto_settings['portfolio-cat-sort-style'] ? 'sort-source-' . esc_attr($porto_settings['portfolio-cat-sort-style']) : 'nav-pills', empty($porto_settings['portfolio-cat-ft']) || empty($portfolio_infinite) ? '' : ' porto-ajax-filter'; ?>">
										<li class="active" data-filter="*"><a href="#"><?php esc_html_e('Show All', 'porto'); ?></a></li>
										<?php foreach ($portfolio_taxs as $portfolio_tax_slug => $portfolio_tax_name): ?>
											<li data-filter="<?php echo esc_attr($portfolio_tax_slug); ?>"><a
													href="<?php echo esc_url(get_term_link($portfolio_tax_slug, 'portfolio_cat')); ?>"><?php echo esc_html($portfolio_tax_name); ?></a>
											</li>
										<?php endforeach; ?>
									</ul>
					...
					if ('timeline' == $portfolio_layout):
						...
						<section class="timeline">
							...
							<?php else: ?>
								...
								<div
									class="clearfix portfolio-row portfolios-container<?php echo 'grid' == $portfolio_layout || 'masonry' == $portfolio_layout ? esc_attr(' portfolio-row-' . $portfolio_columns . ' ' . $portfolio_view) : ''; ?>">
								...
								if (!empty($porto_settings['portfolio-archive-img-lightbox-thumb'])):
									...
									<div class="porto-portfolios-lighbox-thumbnails">
										<div class="owl-carousel owl-theme nav-center"
											data-plugin-options='<?php echo json_encode($thumbs_carousel_options); ?>'>
