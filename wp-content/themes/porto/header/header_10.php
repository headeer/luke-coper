		...
		<div class="header-container container">
			...
			<div class="header-right">
				...
				<div class="header-right-bottom">
					<div id="main-menu">
					<?php
						$menu_html = porto_main_menu();
						if ( $menu_html ) {
							echo porto_filter_output( $menu_html );
						} else {
							echo '<div>' . sprintf( esc_html__( 'Create your first %1$snavigation menu here%2$s and add it to the "Main menu" location.', 'porto' ), '<a href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">', '</a>' ) . '</div>';
						}
					?>
					</div>
					<?php 
