	...
	class Porto_Live_Search {
		...
		public function search_query_by_sku( $where ) {
			...
			foreach ( $skus as $sku ) {
				...
				$parent_ids = $wpdb->get_col( $wpdb->prepare( "SELECT p.post_parent FROM {$wpdb->posts} AS p INNER JOIN {$wpdb->wc_product_meta_lookup} lookup ON p.ID = lookup.product_id AND lookup.sku LIKE '%%%s%%' where p.post_parent != 0 GROUP BY p.post_parent", $sku ) );

				$child_ids_arr = $wpdb->get_results( $wpdb->prepare( "SELECT product_id FROM {$wpdb->wc_product_meta_lookup} WHERE sku LIKE '%%%s%%'", $sku ), ARRAY_N );
