...
<div id="content" role="main">
	...
	<?php else:
		...
		if (!empty($product_ids)) {
			?>
			<div class="align-left heading heading-border heading-middle-border">
				<h3 class="text-uppercase heading-tag font-weight-bold"><?php esc_html_e('Results From Products', 'porto'); ?>
				</h3>
			</div>
			<?php
		...
		if (!empty($portfolio_ids)) {
			?>
			<div class="align-left heading heading-border heading-middle-border">
				<h3 class="text-uppercase heading-tag font-weight-bold"><?php esc_html_e('Results From Portfolios', 'porto'); ?>
				</h3>
			</div>
			<?php
		...
		if (!empty($other_ids)) {
			...
			<div
				class="blog-posts posts-modern posts-container has-ccols ccols-xl-4 ccols-md-3 ccols-sm-2 ccols-1 has-ccols-spacing">
				<?php
				global $post;
...
<?php else: ?>
	...
	<?php if (is_home() && current_user_can('publish_posts')): ?>
		...
		<p class="alert alert-info">
			<?php printf(porto_strip_script_tags(__('Ready to publish your first post? <a href="%1$s">Get started here</a>.', 'porto')), esc_url(admin_url('post-new.php'))); ?>
		</p>

	<?php elseif (is_search()): ?>

		<p class="alert alert-info">
			<?php esc_html_e('Sorry, but nothing matched your search terms. Please try again with different keywords.', 'porto'); ?>
		</p>
		<?php get_search_form(); ?>
	...
	<?php else: ?>
		...
		<p class="alert alert-info">
			<?php esc_html_e('It seems we can&rsquo;t find what you&rsquo;re looking for. Perhaps searching can help.', 'porto'); ?>
		</p>
		<?php get_search_form(); ?>
