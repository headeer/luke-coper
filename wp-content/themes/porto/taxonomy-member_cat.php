		...
		<?php if (have_posts()):
			...
			<div class="<?php echo esc_attr($wrap_cls); ?>" <?php echo porto_filter_output($wrap_attrs); ?>>
						...
						<?php if (function_exists('porto_title_archive_name') && porto_title_archive_name('member')): ?>
							...
							<div class="hide ajax-content-append">
								<h4 class="m-t-sm m-b-lg">
									<?php echo sprintf(esc_html__('More %s:', 'porto'), porto_title_archive_name('member')); ?>
								</h4>
							</div>
						<?php endif; ?>
					</div>
				...
				<?php else: ?>
					...
					<div
						class="member-row members-container row ccols-wrap <?php echo porto_generate_column_classes(isset($porto_settings['member-columns']) ? $porto_settings['member-columns'] : 4); ?>">
						<?php
						while (have_posts()) {
							the_post();
